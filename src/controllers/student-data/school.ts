...
} from "../../models/student-data/school";
import {
  School,
  AddSchoolReqBody,
  GetSchoolsRes,
  SchoolData,
} from "../../types/student-data/school";
import { SimpleRes } from "../../types/shared";

export const addSchool: RequestHandler<
  any,
  SimpleRes,
  AddSchoolReqBody
> = async (req, res, next) => {
  ...
  try {
    ...
    const fetchedSchool = await SchoolModel.findOne({
...
export const getSchools: RequestHandler<
  any,
  GetSchoolsRes | SimpleRes
> = async (req, res, next) => {
  try {
